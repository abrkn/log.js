 ...
 * MIT Licensed
 * 
 * Modifications by J Cupp <jason@cuppster.com>
 * 
 */
...
Log.prototype = {
  ...
  formatDate: function(date) {
    ...
    var pad = function(x) { return ('' + x).length == 2 ? x : '0' + x; };
  ...
  log: function(levelStr, args) {
    if (exports[levelStr] <= this.level) {      
      ...
      , color = colors[levelStr] || colors['DEFAULT'];
      this.stream.write(
        color + '[' + this.formatDate(new Date) + ']'
        + ' ' + levelStr
        + ' ' + msg
        + colors['DEFAULT'] + "\n"
      );
